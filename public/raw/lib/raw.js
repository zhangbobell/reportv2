...
!function (exports){
  ...
  raw.model = function () { 
    ...
    model.instruction = function() {
      return dimensions.values()
        .filter(function(d){ return d.required() > d.value.length; })
        .map(function(d){ var v = d.required() - d.value.length > 1 ? 'dimensions' : 'dimension'; return '<b>'+d.title() + "</b> 至少还需要 " + (d.required() - d.value.length) + " 个维度" ;  })
        .join(". ")
    }
    ...
    ///modified to show dimension name
      model.getDimension = function(){
          return dimensions;
      }
    ///
    return model;
