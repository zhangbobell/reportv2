(function ($) {
    ...
    $.fn.mrjsontable = function (options) {
        ...
        $.each(opts.columns, function (index, item) {
            ...
            if (item.sortable) {
                $("<a>", { "class": "s-init", "href": "javascript: void(0);", "data-i": index, "data-t": item.type, "data-o": item.data}).text(item.heading).bind("click", opts.onSortClick).appendTo($th);
            } else {
        ...
        if (pagingNeeded) {
            ...
            for (var i = 0; i < Math.ceil(opts.data.length / opts.pageSize) ; i++) {
                $("<a>", { "text": " " + (i + 1), "href": "javascript: void(0);", "data-i": (i + 1), "class": "p-link" }).bind("click", opts.onPageClick).appendTo($pager);
            }
